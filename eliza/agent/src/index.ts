...
import { validateAndTweetProposal } from "../custom_actions/validateAndTweetProposal.ts";
...
console.log("Resolved Path to File:", __filename);
...
const startAgents = async () => {
    ...
    const runtimeMap = new Map(); // Store runtimes by agentId

    try {
        for (const character of characters) {
            const runtime = await startAgent(character, directClient);
            runtimeMap.set(character.name, runtime); // Store runtime by name or ID
        }
    } catch (error) {
    ...
    function chat() {
        ...
        const runtime = runtimeMap.get(agentId); // Get the runtime for the agent
        if (!runtime) {
            elizaLogger.error(`No runtime found for agent ${agentId}`);
            process.exit(1);
        }

        rl.question("You: ", async (input) => {
            await handleUserInput(input, agentId, runtime); // Pass runtime explicitly
